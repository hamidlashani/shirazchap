    ...
    <div class="main-content-wrap sidenav-open d-flex flex-column">
        <div class="main-content">
            ...
            <div class="card card-info">
                ...
                <form class="form-horizontal" action="{{ route('admin.printingoffice.updateproduct',['product'=>$product->id]) }}" method="post">
                    @csrf
                    @method('PATCH')
                    <div class="card-body row">
                        <div class="col-4">
                            <div class="form-group col-12">
                                ...
                                <input value="{{ $product->title }}" type="text" name="title" class="form-control" id="inputEmail3" placeholder="عنوان را وارد کنید">
                            </div>
                            ...
                            <div class="form-group col-12">
                                ...
                                <select name="media" class="form-control">
                                    @foreach(\Modules\Printingoffice\Entities\printingofficemedias::all() as $productin)
                                        <option
                                                @php
                                                    if($product->media == $productin->id ){echo 'selected="selected"';}
                            ...
                            <div class="form-group col-12">
                                ...
                                <select name="widths[]" class="form-control select-checkbox" multiple>
                                    @foreach(\Modules\Printingoffice\Entities\printingofficewidths::all() as $width)
                                        ...
                                        <option
                                                @if(in_array($width->id,json_decode($product->widths,true)))
                                                selected="selected"
                                                @endif
                            ...
                            <div class="form-group col-12">
                                ...
                                <div class="input-group">
                                    <input value="{{ $product->image }}" type="text" id="image_label" class="form-control" name="image">
                                    <div class="input-group-append">
                                        <button class="btn btn-outline-secondary" type="button" id="button-image">انتخاب تصویر</button>
                                    </div>
                                </div>
                                <img style="width: 100px;padding: 10px;" src="{{ $product->image }}">
                            ...
                            <div class="form-group col-12">
                                ...
                                <select name="state" class="form-control">
                                    @foreach(\App\Models\state::all() as $state)
                                        <option
                                                @php
                                                    if($product->state == $state->id ){echo 'selected="selected"';}
